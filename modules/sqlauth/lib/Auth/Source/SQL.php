...
class SQL extends \SimpleSAML\Module\core\Auth\UserPassBase
{
    ...
    public function __construct($info, $config)
    {
        ...
        foreach (['dsn', 'username', 'password', 'query', 'hash_column'] as $param) {
        ...
        $this->hashColumn = $config['hash_column'];
    ...
    protected function login(string $username, string $password): array
    {
		...
		$password_hash = $data[0][$this->hashColumn];

		if ( ! password_verify($password, $password_hash)) {
		 /* Invalid password */
		 \SimpleSAML\Logger::error('sqlauthBcrypt:' . $this->authId .
			 ': Hash does not match. Wrong password or sqlauth is misconfigured.');
		 throw new \SimpleSAML\Error\Error('WRONGUSERPASS');
		}
